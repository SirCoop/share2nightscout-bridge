...
function matchTrend(trend) {
  ...
  if (trend in DIRECTIONS) return DIRECTIONS[trend];
...
function fetch_query(opts) {
  ...
  var url =
    (opts.LatestGlucose || Defaults.LatestGlucose) + "?" + qs.stringify(q);
  return url;
}
...
if (!module.parent) {
  if (readENV("API_SECRET").length < Defaults.MIN_PASSPHRASE_LENGTH) {
    var msg = [
      "API_SECRET environment variable should be at least",
      Defaults.MIN_PASSPHRASE_LENGTH,
      "characters",
    ];
